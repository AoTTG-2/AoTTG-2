...
#define _TriplanarFalloff_arr CiconiaStudioCS_StandardBuiltinStandardSpecularsetupCutout
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float2 break26_g476 = uv_BumpMap;
			float GlobalTilingX11 = ( _GlobalXYTilingXYZWOffsetXY.x - 1.0 );
			float GlobalTilingY8 = ( _GlobalXYTilingXYZWOffsetXY.y - 1.0 );
			float2 appendResult14_g476 = (float2(( break26_g476.x * GlobalTilingX11 ) , ( break26_g476.y * GlobalTilingY8 )));
			float GlobalOffsetX10 = _GlobalXYTilingXYZWOffsetXY.z;
			float GlobalOffsetY9 = _GlobalXYTilingXYZWOffsetXY.w;
			float2 appendResult13_g476 = (float2(( break26_g476.x + GlobalOffsetX10 ) , ( break26_g476.y + GlobalOffsetY9 )));
			...
			float3 temp_output_245_0 = UnpackScaleNormal( tex2D( _BumpMap, ( ( appendResult14_g476 + appendResult13_g476 ) + Parallaxe369 ) ), _BumpScale );
			...
			float2 break26_g473 = uv_MainTex;
			float2 appendResult14_g473 = (float2(( break26_g473.x * GlobalTilingX11 ) , ( break26_g473.y * GlobalTilingY8 )));
			float2 appendResult13_g473 = (float2(( break26_g473.x + GlobalOffsetX10 ) , ( break26_g473.y + GlobalOffsetY9 )));
			float4 tex2DNode7_g472 = tex2D( _MainTex, ( ( appendResult14_g473 + appendResult13_g473 ) + Parallaxe369 ) );
			float clampResult27_g472 = clamp( _Saturation , -1.0 , 100.0 );
			float3 desaturateInitialColor29_g472 = ( _Color * tex2DNode7_g472 ).rgb;
			float desaturateDot29_g472 = dot( desaturateInitialColor29_g472, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar29_g472 = lerp( desaturateInitialColor29_g472, desaturateDot29_g472.xxx, -clampResult27_g472 );
			float4 temp_output_338_0 = CalculateContrast(_Brightness,float4( desaturateVar29_g472 , 0.0 ));
			float2 uv_DetailAlbedoMap = i.uv_texcoord * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
			float4 tex2DNode7_g470 = tex2D( _DetailAlbedoMap, uv_DetailAlbedoMap );
			float clampResult27_g470 = clamp( _DetailSaturation , -1.0 , 100.0 );
			float3 desaturateInitialColor29_g470 = ( _DetailColor * tex2DNode7_g470 ).rgb;
			float desaturateDot29_g470 = dot( desaturateInitialColor29_g470, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar29_g470 = lerp( desaturateInitialColor29_g470, desaturateDot29_g470.xxx, -clampResult27_g470 );
			float4 AlbedoDetail153 = CalculateContrast(_DetailBrightness,float4( desaturateVar29_g470 , 0.0 ));
			...
			float4 lerpBlendMode15_g493 = lerp(blendOpDest15_g493,(( blendOpDest15_g493 > 0.5 ) ? ( 1.0 - 2.0 * ( 1.0 - blendOpDest15_g493 ) * ( 1.0 - blendOpSrc15_g493 ) ) : ( 2.0 * blendOpDest15_g493 * blendOpSrc15_g493 ) ),Mask158.x);
				...
				float4 staticSwitch376 = Mask158;
			#else
				float4 staticSwitch376 = (( _BlendmodeOverlay )?( ( saturate( lerpBlendMode15_g493 )) ):( lerpResult343 ));
			#endif
			float4 Albedo26 = staticSwitch376;
			...
			float4 temp_output_248_0 = ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g492 + appendResult13_g492 ) + Parallaxe369 ) ) * _EmissionIntensity );
			...
			float2 break26_g490 = uv_SpecGlossMap;
			...
			float4 tex2DNode3_g489 = tex2D( _SpecGlossMap, ( ( appendResult14_g490 + appendResult13_g490 ) + Parallaxe369 ) );
			float2 uv_DetailSpecGlossMap = i.uv_texcoord * _DetailSpecGlossMap_ST.xy + _DetailSpecGlossMap_ST.zw;
			float4 tex2DNode3_g474 = tex2D( _DetailSpecGlossMap, uv_DetailSpecGlossMap );
			float4 DetailMetallic176 = ( _DetailSpecularColor * tex2DNode3_g474 * _DetailSpecularIntensity );
			float4 lerpResult179 = lerp( ( _SpecularColor * tex2DNode3_g489 * _SpecularIntensity ) , DetailMetallic176 , Mask158);
			float4 Specular41 = lerpResult179;
			o.Specular = Specular41.rgb;
			float BaseColorAlpha46 = (( _InvertABaseColor )?( ( 1.0 - tex2DNode7_g472.a ) ):( tex2DNode7_g472.a ));
			#if defined(_SOURCE_SPECULARALPHA)
				float staticSwitch31_g489 = ( tex2DNode3_g489.a * _Glossiness );
			#elif defined(_SOURCE_BASECOLORALPHA)
				float staticSwitch31_g489 = ( _Glossiness * BaseColorAlpha46 );
			#else
				float staticSwitch31_g489 = ( tex2DNode3_g489.a * _Glossiness );
			#endif
			float DetailBaseColorAlpha170 = tex2DNode7_g470.a;
			#if defined(_DETAILSOURCE_SPECULARALPHA)
				float staticSwitch31_g474 = ( tex2DNode3_g474.a * _DetailGlossiness );
			#elif defined(_DETAILSOURCE_ALBEDOALPHA)
				float staticSwitch31_g474 = ( _DetailGlossiness * DetailBaseColorAlpha170 );
			#else
				float staticSwitch31_g474 = ( tex2DNode3_g474.a * _DetailGlossiness );
			#endif
			float DetailSmoothness175 = staticSwitch31_g474;
			float lerpResult182 = lerp( staticSwitch31_g489 , DetailSmoothness175 , Mask158.r);
			float4 temp_cast_22 = (lerpResult182).xxxx;
			#ifdef _VISUALIZEMASK_ON
				float4 staticSwitch378 = Mask158;
			#else
				float4 staticSwitch378 = temp_cast_22;
			#endif
			float4 Smoothness40 = staticSwitch378;
			o.Smoothness = Smoothness40.r;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float2 break26_g488 = uv_OcclusionMap;
			...
			float blendOpSrc2_g487 = tex2D( _OcclusionMap, ( ( appendResult14_g488 + appendResult13_g488 ) + Parallaxe369 ) ).r;
			float blendOpDest2_g487 = ( 1.0 - _AoIntensity );
			float temp_output_247_0 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g487 ) * ( 1.0 - blendOpDest2_g487 ) ) ));
			...
			float4 tex2DNode8_g494 = tex2D( _CutoutMap, uv_CutoutMap );
			float clampResult24_g494 = clamp( (( _InvertCutout )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2DNode8_g494.r )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2DNode8_g494.r )) )) , 0.0 , 1.0 );
			float Cutout352 = clampResult24_g494;
			clip( Cutout352 - _Cutoff );
		}
