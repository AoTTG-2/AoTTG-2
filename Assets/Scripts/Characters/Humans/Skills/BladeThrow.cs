...
/// </summary>
public class BladeThrow : Photon.MonoBehaviour
{
    ...
    private void Explore()
    {
        ...
        {
            obj2 = (GameObject) Instantiate(Resources.Load("FX/boom6"), base.transform.position, base.transform.rotation);
        }
    ...
    {
        if (GameSettings.Gamemode.Pvp.Mode.Value == PvpMode.Disabled) return;
        ...
        if (((hero != null) && !hero.GetComponent<Hero>().HasDied()) && !hero.GetComponent<Hero>().IsInvincible)
        {
            if ((!hero.GetComponent<Hero>().HasDied()) && !hero.GetComponent<Hero>().IsGrabbed)
            {
                ...
                Debug.Log("blades hit player " + titanName);
