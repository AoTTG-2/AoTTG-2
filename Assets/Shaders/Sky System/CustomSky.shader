...
Shader "Custom/Sky Procedural Stars" {
    Properties{
        ...
        _Gain("Intensity", float) = 3
        _Jitter("Jitter", Range(0,1)) = 1.0

        _Axis("Axis", Vector) = (1, 0, 0, 0)
        _Angle("Angle", float) = 0
        _Threshold("Star Threshold", float) = 0.25
    }
        ...
        SubShader{
            ...
            Pass {
                ...
                v2f vert(appdata_t v)
                {
                    ...
                    OUT.noiseUV = float4(v.vertex.xyz, _Seed);
        ...
        //              for(int i=0; i<int(kSamples); i++)
                ...
                half4 frag(v2f IN) : SV_Target
                {
                    ...
                    // if y < 0 [eyeRay.y > 0] - sky
                        ...
                        float4 uv = IN.noiseUV;

                        uv.rgb = RotateAroundAxis(float3(0, 0, 0), uv.rgb, _Axis, radians(_Angle));

                        half stars = (saturate((pow((1.0 - saturate(fBm_F0(uv, 1))), _Pow) * _Gain)));
                        half4 output = half4(col, 1.0);
                        half bright = (output.r + output.g + output.b) / _Threshold;
                        return output + saturate(stars * saturate(1-bright));
